...
const BedtimeScreen = ({ screenScroll }) => {
  const { setBedTime, setWaterIntake } = useContext(UserContext)
  const [hour, setHour] = useState(22)
  const [minute, setMinute] = useState(20)

  const nextScreen = () => {
    setBedTime(hour.toString() + ' ' + minute.toString())
    setWaterIntake() // normal call
    screenScroll(5)
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable style={styles.backButton} onPress={() => screenScroll(3)}>
